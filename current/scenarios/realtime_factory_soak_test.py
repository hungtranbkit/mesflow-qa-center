...
def _qa_owned_session(row:dict)->bool:
    return str(row.get('start_request_id') or '').startswith(('QA-REAL-START-','QA-RUN-'))


def reconcile_state(c:MESClient,state:dict)->dict:
    """Reconcile persisted state and retain evidence for live QA sessions it cannot safely reconstruct."""
    ...
    for sid,info in list(local.items()):
        ...
        if op_resp.status_code==404 or po_resp.status_code==404:
            ...
            local.pop(sid,None)
    untracked=[]
    for sid,row in live_by_id.items():
        if sid in local or str(row.get('status') or '').upper()!='OPEN' or not _qa_owned_session(row):
            continue
        evidence={
            ...
            'start_request_id':str(row.get('start_request_id') or ''),
            'reason':'live_qa_session_missing_from_persistent_plan',
        }
        untracked.append(evidence)
    state['unexpected_open_sessions']=untracked
    counters=state.setdefault('anomaly_counters',{})
    counters['unexpected_qa_open_sessions']=len(untracked)
    if untracked:
        log('FAIL','STATE-UNTRACKED-QA-OPEN',
            f'Có {len(untracked)} Session QA đang OPEN nhưng mất plan persistent; không tự điền sản lượng',
            sessions=untracked[:30])
    if removed:
...
def start_session(c:MESClient,employee:dict,op:dict,station:dict,device:str,meta:dict,plan:dict,rng:random.Random,state:dict,cal:dict,shift_end:datetime,forgot_finish_rate:float)->bool:
    ...
    intent='EXPECTED-FORGOT' if forgot_finish else ('EXPECTED-PERFORMANCE' if plan.get('profile')!='NORMAL' else 'NORMAL')
    run_id=''.join(ch for ch in str(state.get('test_run_id') or 'UNTRACKED') if ch.isalnum())[:12] or 'UNTRACKED'
    rid=f'QA-RUN-{run_id}-{intent}-{uuid.uuid4().hex[:12]}'
    ...
    counters=state.setdefault('anomaly_counters',{})
    key='expected_anomaly_generated' if forgot_finish else 'normal_session_generated'
    counters[key]=int(counters.get(key,0))+1
    # Persist immediately after each successful server start. A crash later in the
    # worker loop must not orphan an entire batch of OPEN Sessions.
    save_state(state)
...
def inspect_session_exception_outcomes(c:MESClient,state:dict):
    """Compare expected QA anomalies with MESFlow detection instead of treating every warning as test noise."""
    response=c.get('/api/session-exceptions?limit=5000')
    if response.status_code not in (200,201):
        log('WARN','QA-ANOMALY-AUDIT',f'Không đọc được Session Exceptions: HTTP {response.status_code}')
        return
    rows=(_response_body(response).get('items') or [])
    active=[x for x in rows if x.get('is_active') is not False]
    expected={int(sid) for sid,info in state.get('sessions',{}).items() if info.get('forgot_finish')}
    detected={int(x['session_id']) for x in active if x.get('exception_code')=='OPEN_TOO_LONG'}
    qa_detected={int(x['session_id']) for x in active if x.get('data_source')=='QA_TEST'}
    counters=state.setdefault('anomaly_counters',{})
    counters.update({
        'expected_anomaly_detected':len(expected & detected),
        'anomaly_not_detected':len(expected - detected),
        'unexpected_anomaly':len(qa_detected - expected),
    })
    level='PASS' if not counters['anomaly_not_detected'] and not counters['unexpected_anomaly'] else 'FAIL'
    log(level,'QA-ANOMALY-AUDIT','Đối chiếu anomaly dự kiến với MESFlow Session Exceptions',
        expected_generated=int(counters.get('expected_anomaly_generated',0)),
        expected_detected=counters['expected_anomaly_detected'],
        anomaly_not_detected=counters['anomaly_not_detected'],
        unexpected_anomaly=counters['unexpected_anomaly'])
...
def _connect_context(args, state, rng):
    ...
    while True:
        try:
            ...
            if 'AUTH-000 Password required' in str(exc):
                raise RuntimeError('QA_AUTH_CONFIGURATION: cần cấu hình mật khẩu MESFlow trước khi chạy realtime soak') from exc
            attempt+=1
...
def main()->int:
    ...
    p.add_argument('--test-run-id',default='')
    ...
    try:
        ...
        if args.test_run_id: state['test_run_id']=args.test_run_id
        ...
        while now_local()<_campaign_end(state,args.run_days):
            ...
            try:
                ...
                if not last or now-datetime.fromisoformat(last)>=timedelta(minutes=max(5,args.report_interval_minutes)):
                    ...
                    inspect_session_exception_outcomes(c,state)
