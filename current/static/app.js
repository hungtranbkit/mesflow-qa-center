...
function setConnection(ok,text){$('connectionStatus').textContent=text||'';$('connectionBadge').textContent=ok?'Đã kết nối':'Kết nối lỗi';$('connectionBadge').className='pill '+(ok?'ok':'bad');const context=$('contextConnection');if(context){context.textContent=ok?'CONNECTED':'UNAVAILABLE';context.className='pill '+(ok?'ok':'bad')}}
