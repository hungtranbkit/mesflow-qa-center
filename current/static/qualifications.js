...
// ---- Kiosk Certification (ESP Kiosk / Kiosk Certification phase) ----
const KIOSK_YES_NO=v=>v==='PASSED'?'CÓ':v==='FAILED'?'KHÔNG (fail)':v==='BLOCKED'?'Chưa (blocked)':'—';
async function loadKioskCertification(run){
  $('kioskProfileLabel').textContent=run?.profile||'—';
  if(!currentArtifactSha){$('kioskSummary').hidden=true;return}
  try{
    const certBody=await getJson(`/api/qualification/kiosk/certification/${encodeURIComponent(currentArtifactSha)}`);
    const c=certBody.certification.certifications||{};
    $('kioskQSoftware').textContent=KIOSK_YES_NO(c.KIOSK_SOFTWARE_CERTIFIED);
    $('kioskQOffline').textContent=`${KIOSK_YES_NO(c.KIOSK_NETWORK_CERTIFIED)} / ${KIOSK_YES_NO(c.KIOSK_OFFLINE_CERTIFIED)}`;
    $('kioskQHil').textContent=KIOSK_YES_NO(c.KIOSK_HIL_CERTIFIED);
    $('kioskQOta').textContent=KIOSK_YES_NO(c.KIOSK_OTA_CERTIFIED);
  }catch(error){$('kioskQSoftware').textContent='Không tải được';}
  if(!currentRunId){$('kioskCategoryRows').innerHTML='<tr><td colspan="5" class="empty-cell">Chưa có run.</td></tr>';return}
  try{
    const body=await getJson(`/api/qualification/runs/${encodeURIComponent(currentRunId)}/kiosk`);
    const cats=body.categories||[];
    $('kioskCategoryRows').innerHTML=cats.length?cats.map(g=>`<tr><td>${esc(g.category_label)}</td><td>${g.passed}</td><td>${g.failed}</td><td>${g.blocked}</td><td>${g.total}</td></tr>`).join(''):'<tr><td colspan="5" class="empty-cell">Chưa có kết quả kiosk cho run này.</td></tr>';
    const items=cats.flatMap(g=>g.items);
    $('kioskTestcaseRows').innerHTML=items.length?items.map(x=>`<tr><td>${esc(x.testcase_id||'—')}</td><td>${esc(x.name)}</td><td><span class="status ${statusClass(x.status)}">${esc(x.status)}</span></td><td>${esc(x.started_at||'—')}</td><td>${esc(x.first_failing_step||'—')}</td></tr>`).join(''):'<tr><td colspan="5" class="empty-cell">—</td></tr>';
  }catch(error){$('kioskCategoryRows').innerHTML=`<tr><td colspan="5" class="empty-cell">Không tải được kết quả kiosk: ${esc(error.message)}</td></tr>`}
}
async function triggerKioskRun(profile,btn){
  if(!currentArtifactSha){alert('Chưa có artifact được chọn.');return}
  btn.disabled=true;const original=btn.textContent;btn.textContent='Đang chạy…';
  try{
    const started=await postJson('/api/qualification/kiosk/run',{artifact_sha256:currentArtifactSha,profile});
    const job=await pollJobUntilDone(started.job.id);
    if(job.status!=='SUCCESS')throw new Error(job.error||`job kết thúc với trạng thái ${job.status}`);
    await refresh();
  }catch(error){$('qualificationError').hidden=false;$('qualificationError').textContent=`Kiosk ${profile} test thất bại: ${error.message}`}
  finally{btn.disabled=false;btn.textContent=original}
}
document.querySelectorAll('[data-kiosk-run]').forEach(btn=>btn.onclick=()=>triggerKioskRun(btn.dataset.kioskRun,btn));
...
async function selectRun(run,certs){const button=$('refreshQualification');button.disabled=true;document.body.setAttribute('aria-busy','true');currentRunId=run.id;currentArtifactSha=run.artifact_sha256;try{const detail=await getJson(`/api/qualification/runs/${encodeURIComponent(run.id)}`);const matching=certs.find(c=>c.artifact_id===run.artifact_id&&c.environment_id===run.environment_id);renderRun(detail.run);renderCertification(matching);await loadCoverage();await loadKioskCertification(detail.run);await loadPolicyTiers();await refreshSandboxesForCurrentRun();updateLivePanelForRun(detail.run);$('qualificationError').hidden=true;$('qualificationError').textContent=''}catch(error){$('qualificationError').hidden=false;$('qualificationError').textContent=`Không tải được qualification evidence: ${error.message}`}finally{button.disabled=false;document.body.removeAttribute('aria-busy')}}
