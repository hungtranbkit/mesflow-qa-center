...
def session_from_config(cfg: dict[str, Any]) -> requests.Session:
    ...
    if not s.verify:
        ...
        try:
            ...
            pass
    s.headers.update({"User-Agent": "MESFlow-QA-Center/1.19.5", "Accept": "application/json,text/html"})
...
@app.get("/api/version")
def api_version():
    return jsonify({"ok": True, "app": "MESFlow QA Center", "version": APP_VERSION, "mode": "internal-only" if QA_INTERNAL_ONLY else "custom", "target": QA_INTERNAL_URL if QA_INTERNAL_ONLY else ""})
